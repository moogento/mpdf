...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	public function __construct(array $config = [])
	{
		...
		$this->noImageFile = __DIR__ . '/../data/no_image.png';
		$this->subPos = 0;
		...
		foreach ($this->fontdata as $f => $fs) {
			...
			if (isset($fs['SB']) && $fs['SB']) {
				$this->available_unifonts[] = $f . 'SB';
			}
			if (isset($fs['I']) && $fs['I']) {
			...
			if (isset($fs['SBI']) && $fs['SBI']) {
				$this->available_unifonts[] = $f . 'SBI';
			}
		}
	...
	// mPDF 5.7.1
		...
		for ($i = 0; $i < $clen; $i++) {
			...
			if (( $contentWidth + $cw > $maxWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Mpdf::SCALE) ) + 0.001)) {// 0.001 is to correct for deviations converting mm=>pts
				...
				$currContent = rtrim((string)$currContent);
				if ($this->checkCJK) {
	...
	function _createHTMLheaderFooter($arr, $hf)
	{
		...
		if (isset($arr['L']['font-style'])) {
			...
			if ($arr['L']['font-style'] == 'SB' || $arr['L']['font-style'] == 'SBI') {
		...
		if (isset($arr['C']['font-style'])) {
			...
			if ($arr['C']['font-style'] == 'SB' || $arr['C']['font-style'] == 'SBI') {
				$html .= ' font-weight: 400;';
			}
		...
		if (isset($arr['R']['font-style'])) {
			...
			if ($arr['R']['font-style'] == 'SB' || $arr['R']['font-style'] == 'SBI') {
				$html .= ' font-weight: 400;';
			}
			...
			if (strlen($Harray) === 0) {
				...
				if ($this->defaultheaderfontstyle) {
					...
					if ($this->defaultheaderfontstyle == 'B' || $this->defaultheaderfontstyle == 'BI') {
						$oddhtml .= ' font-weight: 600;';
					}

					if ($this->defaultheaderfontstyle == 'SB' || $this->defaultheaderfontstyle == 'SBI') {
						$oddhtml .= ' font-weight: 400;';
					}
				...
				if ($this->defaultheaderfontstyle) {
					if ($this->defaultheaderfontstyle == 'B' || $this->defaultheaderfontstyle == 'BI') {
						$evenhtml .= ' font-weight: 600;';
					}
					if ($this->defaultheaderfontstyle == 'SB' || $this->defaultheaderfontstyle == 'SBI') {
						$evenhtml .= ' font-weight: 400;';
					}
					if ($this->defaultheaderfontstyle == 'I' || $this->defaultheaderfontstyle == 'BI' || $this->defaultheaderfontstyle == 'SBI') {
		...
		if (is_string($Farray)) {
			...
			if (strlen($Farray) == 0) {
				...
				if ($this->defaultfooterfontstyle) {
					...
					if ($this->defaultfooterfontstyle == 'SB' || $this->defaultfooterfontstyle == 'SBI') {
						$oddhtml .= ' font-weight: 400;';
					}
					if ($this->defaultfooterfontstyle == 'I' || $this->defaultfooterfontstyle == 'BI' || $this->defaultfooterfontstyle == 'SBI') {
						$oddhtml .= ' font-style: italic;';
					}
				}
	...
	function ClearFloats($clear, $blklvl = 0)
	{
		...
		$this->ResetMargins();
		$this->pageoutput[$this->page] = [];
		$this->y = (((int)$end * 1000) % 1000000) / 1000; // mod changes operands to integers before processing
	}
